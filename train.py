...
def train(data_loader, net, criterion, optimizer,epoch):
    ...
    for i, (img, pt, lbl) in enumerate(data_loader):
        img = img.cuda()
        pt = pt.cuda()
        lbl = lbl.cuda()
        data = (img, pt)
        ...
        if i % 10==0:
           print(f"\t[{i}/{len(data_loader)}], Loss {loss.item():.4f}  ,total_loss  {Loss1:.4f}")
           Loss1=0
...
def validation(data_loader, net, epoch):
    ...
    net.eval()
    all_lbls, all_preds = [], []
    fts_img, fts_mesh, fts_pt, fts_vox ,fts_features= [], [], [], [], []

    st = time.time()
    for img, pt , lbl in data_loader:
        img = img.cuda()
        pt = pt.cuda()
        lbl = lbl.cuda()
        data = (img, pt)
