...
ckpt_path = 'cache/ckpts/----------/ckpt1.pth'
####################################
...
def extract(query_loader, target_loader, net):
    ...
    q_fts_all = np.concatenate(q_fts_all, axis=0)
    q_fts_uni = q_fts_all

    for img, pt in target_loader:
        img = img.cuda()
        pt = pt.cuda()
        data = (img, pt)
        _, ft = net(data, global_ft=True)
    ...
    t_fts_all = np.concatenate(t_fts_all, axis=0)
    t_fts_uni = t_fts_all
